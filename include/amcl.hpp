...
public:
    ...
    void update(const std::vector<double>& actual_scan)
    {
        ...
        for (auto& p : particles_)
        {
            ...
            std::vector<double> p_scan = simulate_scan_(p.pose, actual_scan.size());
...
private:
    ...
    std::vector<double> simulate_scan_(const Pose& p, int num_beams)
    {
        ...
        for (int i = 0; i < num_beams; ++i)
        {
            ...
            {
                range += 0.05;
                if (map_.is_wall_at_physical(p.x + range * std::cos(angle), p.y + range * std::sin(angle)))
                {
                    break;
                }
            }
